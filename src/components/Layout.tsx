...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans flex flex-col md:flex-row">
      {/* Mobile Header */}
      <div className="md:hidden flex items-center justify-between p-4 bg-white border-b border-red-50 sticky top-0 z-50">
      ...
      <AnimatePresence>
        {(isSidebarOpen || true) && (
          <motion.aside
            ...
            animate={{ x: 0 }}
            className={`
              fixed md:sticky top-0 left-0 h-screen w-72 bg-slate-50 border-r border-red-50 z-40 
              flex flex-col transform transition-transform duration-300 ease-in-out
              ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
            `}
          >
            <div className="p-8 border-b border-red-50 bg-white">
                  ...
                  {[...REGIONS].sort((a, b) => a.name.localeCompare(b.name)).map((region) => (
                    <li key={region.id}>
                      <Link
                        ...
                        onClick={() => setIsSidebarOpen(false)}
                        className={`
                          flex flex-col gap-0.5 px-3 py-2 rounded-lg transition-all
                          ${location.pathname === `/filia/${region.id}`
                            ? 'bg-blue-50 text-blue-900 border border-blue-100/50'
                            : 'hover:bg-blue-50/50 text-slate-500 hover:text-blue-900'}
                        `}
                      >
                        <div className="flex items-center justify-between gap-1">
                          <span className="text-xs font-serif font-bold italic">{region.name}</span>
                          <ChevronRight size={10} className="opacity-30" />
                        </div>
                        <span className="text-[8px] opacity-60 leading-tight truncate uppercase tracking-tighter">
                          {region.cities}
                        </span>
                      </Link>
                    </li>
            ...
            <div className="p-6 border-t border-red-50 bg-white mt-auto">
              <div className="flex items-center gap-4 justify-center">
                <a href="https://www.facebook.com/SkandiiKuzniaTalentow" target="_blank" rel="noopener noreferrer" className="p-2 bg-white rounded-full text-blue-900 border border-blue-100 hover:bg-red-600 hover:text-white hover:border-red-600 transition-all shadow-sm">
                  <Facebook size={18} />
                </a>
                <a href="mailto:post@skandiatalentsmie.no" className="p-2 bg-white rounded-full text-blue-900 border border-blue-100 hover:bg-red-600 hover:text-white hover:border-red-600 transition-all shadow-sm">
                  <Mail size={18} />
                </a>
              </div>
            </div>
      ...
      <main className="flex-1 min-h-screen pt-0 md:pt-0">
        ...
        {/* Footer */}
        <footer className="w-full border-t border-red-50 bg-white mt-12 py-16">
