...
import { ArrowRight, CheckCircle, Heart, Sparkles, Lightbulb, Gift } from 'lucide-react';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-24 pb-20">
      <header className="space-y-8 text-center">
        <h1 className="text-4xl sm:text-5xl md:text-7xl font-serif font-bold tracking-tight text-slate-900">
          {t('join.title').split('!')[0]} <span className="text-blue-600">{t('join.title').split('!')[1] || ''}</span>! 📢
        </h1>
        ...
        {CATEGORIES.map((cat, idx) => {
          ...
          return (
            <div key={idx} className="bg-white p-8 rounded-[3rem] border border-blue-50 shadow-sm transform hover:-translate-y-2 transition-all duration-500 flex flex-col group">
              ...
              <h3 className="text-2xl font-serif font-bold mb-4 text-slate-900">{t(cat.titleKey)}</h3>
              <ul className="space-y-3 flex-1">
                {t(cat.itemsKey).split(';').map((item, i) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        <section className="bg-slate-50 p-12 rounded-[4rem] border border-blue-50 space-y-10 shadow-inner">
          <h2 className="text-3xl sm:text-4xl font-serif font-bold flex items-center gap-4 text-slate-900">
            <Lightbulb className="text-blue-600 flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12" /> {t('join.why.title')}
          </h2>
          <div className="space-y-6">
            {[
              ...
              <div key={i} className="flex gap-4">
                <CheckCircle className="text-sky-600 flex-shrink-0 mt-1" size={24} />
        ...
        <section className="bg-white p-8 sm:p-12 rounded-[4rem] border border-blue-50 space-y-10 shadow-sm transform rotate-1">
          <h2 className="text-3xl sm:text-4xl font-serif font-bold flex items-center gap-4 text-slate-900">
            <Gift className="text-blue-600 flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12" /> {t('join.offer.title')}
          </h2>
      ...
      >
        <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-900">{t('join.steps.title')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {[
            { step: "1", text: t('join.steps.1') },
            { step: "2", text: t('join.steps.2') },
            { step: "3", text: t('join.steps.3') }
          ].map((item, i) => (
      ...
      {/* Footer Section - Shared with Contact style but for JoinUs */}
      <div className="bg-blue-600 text-white p-16 md:p-24 rounded-[5rem] text-center space-y-12 shadow-2xl shadow-blue-900/20 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <h2 className="text-3xl sm:text-5xl md:text-7xl font-serif font-bold italic">{t('membership.title')}</h2>
        ...
        <div className="pt-16 border-t border-white/20 flex flex-col items-center gap-10 relative z-10">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm opacity-80 uppercase tracking-widest font-bold">{t('contact.mainEmail')}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-8 w-full max-w-3xl">
            ...
            >
              ...
              <div className="text-center space-y-1">
                <p className="text-[10px] uppercase tracking-widest opacity-60 font-bold">{t('contact.facebook')}</p>
                <p className="text-xl font-serif font-bold italic">{t('common.visitFacebook')}</p>
              </div>
            </a>
            <a 
            ...
            >
              ...
              <div className="text-center space-y-1">
                <p className="text-[10px] uppercase tracking-widest opacity-60 font-bold">{t('nav.join')}</p>
                <p className="text-xl font-serif font-bold italic">{t('home.fbGroup')}</p>
              </div>
            </a>
          </div>
        </div>
